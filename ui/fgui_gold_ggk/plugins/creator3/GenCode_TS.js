...
function genCode(handler) {
    ...
    for (let i = 0; i < classCnt; i++) {
        ...
        let desc = handler.GetItemDesc(classInfo.res);
        if (desc != null) {
            console.log(typeof desc);
            let hasLang = false;
            if (typeof desc === 'object') {
                let lang_obj = {};
                for (let a = 0; a < desc.elements.rawList.Count; a++) {
                    let elements = desc.elements.rawList.get_Item(a).elements;
                    console.log("elements Count : " + elements.Count);
                    for (let b = 0; b < elements.Count; b++) {
                        let element = elements.rawList.get_Item(b);
